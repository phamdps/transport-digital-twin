...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.chnanel_independence = configs.channel_independence
    ...
    def forward(self, x):
        ...
        if self.individual:
            ...
            if self.chnanel_independence:
                ...
                seasonal = seasonal_init.reshape(B, -1)
                trend = trend_init.reshape(B, -1)
                seasonal_output = self.Linear_Seasonal(seasonal)
                trend_output = self.Linear_Trend(trend) 
        if self.chnanel_independence:     
            x = seasonal_output + trend_output
        else:
            x = (seasonal_output + trend_output).view(B, self.channels, self.pred_len)
